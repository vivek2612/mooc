...
xlabel = reqType+" : Req-size(in Bytes)"
...
if reqType=="video":
	result = plotHistogram1(reqSizeArr, True, tuple(l), False, False, saveflag, xlabel, fname+title, legendLoc)
else:
	result = plotHistogram(reqSizeArr, True, tuple(l), False, False, saveflag, xlabel, fname+title, legendLoc)
